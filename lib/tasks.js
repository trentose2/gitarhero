...
Se TYPE è 'Open question':
    - ha un campo ANSWERS che rappresenta una POTENZIALE risposta corretta
...
const postTasks = (req, res) => {
    ...
    if (!task.question || typeof task.question != 'string') {
        res.status(400).json({ error: 'The field "question" is mandatory and must be a string' });
        return;
    }
    ...
    /* Una domanda aperta impone che il campo "answers" sia una stringa */
    if (task.type == 'Open question' && typeof task.answers != 'string') {
        res.status(400).json({ error: 'If type is "Open question", field "answers" must be a string' });
        return;
    }
    ...
    if (task.type == 'Multiple choice' && !Array.isArray(task.answers)) {
        res.status(400).json({ error: 'If type is "Multiple choice", field "answers" must be an array' });
        return;
    }
